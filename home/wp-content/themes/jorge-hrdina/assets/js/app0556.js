...
jQuery(document).ready(function($){
    ...
    if($('body').hasClass('home')){
      $('.element__white').attr('data-elementcolor', '#FFFFFF').attr('data-bgcolor', '#000000');
      $('.element__black').attr('data-elementcolor', '#000000').attr('data-bgcolor', '#FFFFFF');
...
function initMap() {
  ...
  const iconBase ='https://jorgehrdinadev.wpengine.com/wp-content/themes/jorge-hrdina/assets/images/';
...
window.addEventListener("load", function() {
  ...
  if(jQuery("body").hasClass("home")){
    jQuery(".fusion-header-wrapper .fusion-header").css("background-color", "#000000");
    jQuery(".fusion-toggle-icon-line").css("background-color", "#FFFFFF");
  }
  

  const scrollBgColor = document.querySelectorAll("[data-bgcolor]");
  scrollBgColor.forEach((bgColorHeader, i) => {
    const prevBgColor = i === 0 ? "#FFFFFF" : scrollBgColor[i - 1].dataset.bgcolor;
    ScrollTrigger.create({
      trigger: bgColorHeader,
      start: "top 9%",
      onEnter: () => {
        if(jQuery(".fusion-header-wrapper").hasClass("fusion-is-sticky")){
          gsap.to(".fusion-header", {backgroundColor: bgColorHeader.dataset.bgcolor, overwrite: 'true'})
        }
      },
      onLeaveBack: () => {
        if(jQuery(".fusion-header-wrapper").hasClass("fusion-is-sticky")){
          gsap.to(".fusion-header", {backgroundColor: prevBgColor, overwrite: 'true'})
        }
      }
    });
  });
  ...
  scrollHeaderElems.forEach((colorElements, j) => {
    const prevElementColor = j === 0 ? "#000000" : scrollHeaderElems[j - 1].dataset.elementcolor;
    ScrollTrigger.create({
      trigger: colorElements,
      start: "top 9%",
      onEnter: () => {
        gsap.to(".fusion-toggle-icon-line", {backgroundColor: colorElements.dataset.elementcolor, overwrite: 'auto'})
        gsap.to(".logo__svg path", {fill: colorElements.dataset.elementcolor, overwrite: 'auto'})
        if(jQuery(".fusion-header-wrapper").hasClass("fusion-is-sticky")){
          gsap.to("#menu-main-menu .menu-text", {color: colorElements.dataset.elementcolor, overwrite: 'auto'})
        }

      },
      onLeaveBack: () => {
        gsap.to(".fusion-toggle-icon-line", {backgroundColor: prevElementColor, overwrite: 'auto'})
        gsap.to(".logo__svg path", {fill: prevElementColor, overwrite: 'auto'})    
        if(jQuery(".fusion-header-wrapper").hasClass("fusion-is-sticky")){
          gsap.to("#menu-main-menu .menu-text", {color: prevElementColor, overwrite: 'auto'})
        }
      } 
    });
  });
